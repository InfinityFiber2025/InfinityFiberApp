...
function criarAgendamentoInicial200k(){
  ...
  clientes.forEach(c=>{
    const t = {
      id: self.crypto ? crypto.randomUUID() : String(Math.random()),
      data: agora.toISOString(),
      tipo: 'CRÉDITO INICIAL',
      valor: 200000,
      de: 'Infinity Fiber Bank',
      para: c.email,
      status: 'pendente_aprovacao', // pendente_aprovacao -> aprovada_aguardando -> concluída
...
function abrirCofre(){
  ...
  $("#conteudo").innerHTML=`<h3>Saldo do Cofre: ${moeda(cofreAgencia)}</h3><p>Banco ${CONFIG.banco} — Agência ${CONFIG.agencia}</p>`;
}

// ===== Admin: Clientes =====
function abrirClientesAdmin(){
  $("#titulo").textContent="Clientes";
  let html="<table class='table'><tr><th>Cliente</th><th>Conta</th><th>Saldo</th><th>Status</th></tr>";
  clientes.forEach(c=>{
    html+=`<tr>
...
// ===== Admin: Transações (com biometria) =====
function abrirTransacoesAdmin(){
  ...
  const linha = (t)=>`<tr>
    ...
    <td>${t.para}</td>
    <td>${moeda(t.valor)}</td>
    <td><span class="badge">${statusLabel(t.status)}</span></td>
    <td>${t.scheduledFor?formatDate(new Date(t.scheduledFor)):'-'}</td>
    <td>${t.status==='pendente_aprovacao' ? `<button class="btn small" onclick="aprovarTransacaoBiometria('${t.id}')">Aprovar por Biometria</button>` : '-'}</td>
  </tr>`;
  ...
  $("#conteudo").innerHTML = `
    ...
    ${pendentes.map(linha).join('') || '<tr><td colspan="7">Nenhuma pendente.</td></tr>'}</table>

    <h3>Aprovadas (Aguardando Janela Bancária)</h3>
    <table class="table"><tr><th>Tipo</th><th>Origem</th><th>Destino</th><th>Valor</th><th>Status</th><th>Agendada p/</th><th>Ação</th></tr>
    ${aprovadas.map(linha).join('') || '<tr><td colspan="7">Nenhuma.</td></tr>'}</table>

    <h3>Concluídas</h3>
    <table class="table"><tr><th>Tipo</th><th>Origem</th><th>Destino</th><th>Valor</th><th>Status</th><th>Data/Hora</th><th></th></tr>
    ${concluidas.map(t=>`<tr><td>${t.tipo}</td><td>${t.de}</td><td>${t.para}</td><td>${moeda(t.valor)}</td><td><span class="badge">${statusLabel(t.status)}</span></td><td>${formatDate(new Date(t.data))}</td><td>-</td></tr>`).join('') || '<tr><td colspan="7">Nenhuma.</td></tr>'}
    </table>
...
function statusLabel(s){
  return s==='pendente_aprovacao' ? 'Pendente de aprovação'
    : s==='aprovada_aguardando' ? 'Aprovada — aguardando janela'
    : s==='concluída' ? 'Concluída'
    : s;
}
    ...
    if(t.status==='aprovada_aguardando' && t.scheduledFor){
      ...
      if(now>=quando && dentroJanela(now)){
        ...
        if(dest && dest.status==='ATIVA'){
          dest.saldo += t.valor;
          dest.extrato.push({ data: new Date().toISOString(), tipo:t.tipo, de:t.de, para:t.para, valor:t.valor, status:'concluída' });
          t.status='concluída';
          t.data = new Date().toISOString();
          cofreAgencia -= t.valor;
        }
      }
    }
  });
...
function abrirExtratoCliente(){
  ...
  movs.slice().reverse().forEach(t=>{
    html += `<tr><td>${formatDate(new Date(t.data))}</td><td>${t.tipo}</td><td>${t.de}</td><td>${moeda(t.valor)}</td><td><span class='badge'>${t.status}</span></td></tr>`;
  });
  html += "</table>";
