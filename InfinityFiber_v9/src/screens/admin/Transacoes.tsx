...
const DATA=[{id:'t1',cliente:'Maria Santos',valor:250.00,status:'Pendente'},{id:'t2',cliente:'João Almeida',valor:85.90,status:'Pendente'},{id:'t3',cliente:'Carla Nunes',valor:1000.00,status:'Aprovada'}];
export default function Transacoes({navigation}:any){
  return (<View style={styles.container}>
    ...
    <FlatList data={DATA} keyExtractor={i=>i.id} renderItem={({item})=>(
      ...
      <View style={{flexDirection:'row',gap:8}}><Botao title="Aprovar" onPress={()=>navigation.navigate('BiometriaAdmin')}/><Botao title="Recusar"/></View></Card>
    )}/>
  </View>);
}
